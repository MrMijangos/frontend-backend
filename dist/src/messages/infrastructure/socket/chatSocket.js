...
function setupChatSocket(io, sendMessageUseCase, getLobbyMessagesUseCase) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send_message', async (data) => {
            try {
                const message = await sendMessageUseCase.execute(data.lobby_id, socket.data.userId, data.content);
